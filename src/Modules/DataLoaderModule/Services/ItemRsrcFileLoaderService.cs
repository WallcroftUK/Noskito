...
{
    public class ItemRsrcFileLoaderService : RsrcProv<ItemDTO>
    {
        private const string FILE_NAME = "Item.dat";
        private readonly RsrcConfigLoader _configuration;
        private readonly List<ItemDTO> _itemDtos = new();

        public ItemRsrcFileLoaderService(RsrcConfigLoader configuration)
        {
            _configuration = configuration;
        }

        public async Task<IReadOnlyList<ItemDTO>> FetchAsync()
        {
            if (_itemDtos.Any())
            {
                return _itemDtos;
            ...
            string filePath = Path.Combine(_configuration.DataPath, FILE_NAME);
            ...
            while ((line = await npcIdStream.ReadLineAsync()) != null)
            {
                try
                {
                    ...
                    ProcessCurrentLine(currentLine, item, ref itemAreaBegin, ref itemCounter);
                }
                catch (Exception e)
                {
            ...
            Log.Info($"[DATA_LOADER] {itemCounter.ToString()} Items loaded");
            return _itemDtos;
        }

        private void ProcessCurrentLine(string[] currentLine, ItemDTO item, ref bool itemAreaBegin, ref int itemCounter)
        {
            switch (currentLine.Length)
            {
                ...
                case > 1 when currentLine[1] == "END":
                    ProcessEndCase(currentLine, item, ref itemAreaBegin, ref itemCounter);
                    break;
                ...
                case > 3 when currentLine[1] == "TYPE":
                    ProcessTypeCase(currentLine, item);
                    break;
        ...
        private void ProcessEndCase(string[] currentLine, ItemDTO item, ref bool itemAreaBegin, ref int itemCounter)
        {
            if (!itemAreaBegin)
            {
                return;
        ...
        private void ProcessTypeCase(string[] currentLine, ItemDTO item)
        {
